...
var (
	...
	ErrFailedToLoginUser  = errors.New("failed to login")
)
...
func (s *Service) Login(ctx context.Context, credentials models.Credentials) (*models.AuthTokens, error) {
	...
	if err != nil {
		log.Error("error to check user existence", slog.String("operation", op), sl.Err(err))
		return nil, ErrFailedToLoginUser
	}

	if err = hash.CompareHashAndPassword(candidate.Password, credentials.Password); err != nil {
